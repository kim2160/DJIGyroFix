...
2. `파일 선택`에서 DJI 카메라 원본 MP4/MOV를 선택합니다.
3. 시작 시간과 종료 시간을 입력합니다.
4. 필요한 경우 `+`로 구간을 추가합니다. 최대 10개까지 입력할 수 있습니다.
5. `검출`로 흔들림 위치를 확인하거나 바로 `FIX`를 실행합니다.
6. 원본 폴더의 `원본이름_gyro_fixed.MP4`를 확인합니다.
